~function () {
    ...
    //->move:实现多方向的运动动画 curEle->当前要操作运动的元素 target->当前动画的目标位置,
    //  存储的是每一个方向的目标位置{left:xxx,top:xxx,...} duration->当前动画的总时间
    function move(curEle, target, duration, effect, callBack) {
        ...
        if (!isNaN(effect)) {
            ...
            effect = effect.split("-");
            effect = effect.length >= 2 ? Effect[effect[0]][effect[1]] : Effect[effect[0]];
